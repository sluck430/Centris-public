...
# 新增
def extractVerDate_new(repoName):
    # For extracting version (tag) date

    verDateDict = {}
    if os.path.isfile(os.path.join(tagDatePath, repoName)):
        with open(file=os.path.join(tagDatePath, repoName), mode='r', encoding='UTF-8') as f:
            verDateDict = json.load(f)
...
def redundancyElimination():
    for repoName in os.listdir(resultPath):
        ...
        funcDateDict = {}
        tempDateDict = {}
        verDateDict = extractVerDate_new(repoName)
